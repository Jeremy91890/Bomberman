...
{
    if (game->player_infos[actual]->alive == 1)
    {
        if (game->game_state == 1)
        {
            if (req.dir != 0 && req.dir != game->player_infos[actual]->current_dir)
            {
                turn_player(game, actual, req);
            }
            else if (req.x_pos != game->player_infos[actual]->x_pos || req.y_pos != game->player_infos[actual]->y_pos)
            {
