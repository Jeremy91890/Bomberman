...
void display_character(t_game *game)
{
    ...
    while (i < (MAX_PLAYERS))
    {
        ...
        if (game->player_infos[i]->alive == 1)
        {
            ...
            {
            case 0:
                switch (game->player_infos[i]->current_dir)
                {
                case 1:
                    Rect_dest.x = game->player_infos[i]->x_pos * BMAN_WIDTH;
                    Rect_dest.y = (game->player_infos[i]->y_pos * (BMAN_HEIGHT / 2)) - (BMAN_HEIGHT / 2);
